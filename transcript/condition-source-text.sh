...
parser.add_argument('-d', '--dummy-token', default='XXXXXXXXXX', help='token to print when all tokens have been removed from a line')
parser.add_argument('-min', '--min-length', default=0, help='minimum transcript length to include (in token count)')
parser.add_argument('-max', '--max-length', default=sys.maxint, help='maximum transcrip length to include (in token count)')
